...
<body class="scrapbook-paper linen-texture min-h-screen text-espresso font-sans p-2 sm:p-4 md:p-8 flex flex-col justify-between relative overflow-x-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 my-auto items-start">
            ...
            <div class="md:col-span-7 bg-[#F4EFE6] border-2 border-espresso p-4 sm:p-6 rounded-xl shadow-md relative flex flex-col items-center gap-4">
                ...
                <div class="flex gap-4 justify-center w-full">
                    <div id="slot-btn-0" data-slot="0" class="slot-trigger w-24 aspect-[4/3] bg-espresso/10 border-2 border-dashed border-espresso rounded overflow-hidden relative cursor-pointer flex items-center justify-center hover:bg-espresso/20 transition">
                        <img id="thumb-0" class="w-full h-full object-cover hidden pointer-events-none" alt="Foto 1">
                        <span id="label-0" class="font-hand text-xs text-cocoa-medium text-center font-bold pointer-events-none">📸 FOTO 1</span>
                        <input type="file" id="photo-input-0" data-slot="0" accept="image/*" class="slot-file-input hidden">
                    </div>
                    <div id="slot-btn-1" data-slot="1" class="slot-trigger w-24 aspect-[4/3] bg-espresso/10 border-2 border-dashed border-espresso rounded overflow-hidden relative cursor-pointer flex items-center justify-center hover:bg-espresso/20 transition">
                        <img id="thumb-1" class="w-full h-full object-cover hidden pointer-events-none" alt="Foto 2">
                        <span id="label-1" class="font-hand text-xs text-cocoa-medium text-center font-bold pointer-events-none">📸 FOTO 2</span>
                        <input type="file" id="photo-input-1" data-slot="1" accept="image/*" class="slot-file-input hidden">
                    </div>
                    <div id="slot-btn-2" data-slot="2" class="slot-trigger w-24 aspect-[4/3] bg-espresso/10 border-2 border-dashed border-espresso rounded overflow-hidden relative cursor-pointer flex items-center justify-center hover:bg-espresso/20 transition">
                        <img id="thumb-2" class="w-full h-full object-cover hidden pointer-events-none" alt="Foto 3">
                        <span id="label-2" class="font-hand text-xs text-cocoa-medium text-center font-bold pointer-events-none">📸 FOTO 3</span>
                        <input type="file" id="photo-input-2" data-slot="2" accept="image/*" class="slot-file-input hidden">
                    </div>
                </div>
                ...
                <div class="w-full bg-[#FAF7F2] p-4 rounded-lg border border-cocoa-light/20 flex flex-col gap-3 text-left">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="text" id="header_text" value="Capturing Moments" class="cfg-input text-xs bg-cream-light p-2 rounded border border-cocoa-light focus:outline-none">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="font-serif text-[10px] font-bold text-espresso">Tulisan Bawah (Footer):</label>
                            <input type="text" id="footer_text" value="On the road, 20!" class="cfg-input text-xs bg-cream-light p-2 rounded border border-cocoa-light focus:outline-none">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="font-serif text-[10px] font-bold text-espresso">Gaya/Style Strip Foto:</label>
                            <select id="style_theme" class="cfg-input text-xs bg-cream-light p-2 rounded border border-cocoa-light focus:outline-none">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select id="photo_shape" class="cfg-input text-xs bg-cream-light p-2 rounded border border-cocoa-light focus:outline-none">
                ...
                <div class="flex flex-wrap gap-3 w-full justify-center mt-1">
                    <button id="start-camera-btn" type="button" class="relative z-20 px-5 py-2.5 bg-tiramisu-dark text-espresso font-serif font-bold rounded-lg border-2 border-espresso shadow hover:bg-tiramisu-light transition cursor-pointer">
                        📹 AKTIFKAN KAMERA
                    </button>
                    <button id="snap-btn" type="button" class="relative z-20 px-5 py-2.5 bg-espresso text-cream-light font-serif font-bold rounded-lg shadow hover:bg-cocoa-medium transition hidden cursor-pointer">
    ...
    (function() {
        ...
        function drawRoundedRect(cCtx, x, y, width, height, radius, fillStyle, strokeStyle, lineWidth) {
            ...
            if (fillStyle) { cCtx.fillStyle = fillStyle; cCtx.fill(); }
            if (strokeStyle) { cCtx.strokeStyle = strokeStyle; cCtx.lineWidth = lineWidth; cCtx.stroke(); }
        }
        ...
        function drawBarcode(cCtx, x, y, width, height, color) {
            ...
            while (currX < x + width) {
                ...
                var barW = Math.floor(2 + (seed / 233280) * 5);
                cCtx.fillRect(currX, y, barW, height);
                seed = (seed * 9301 + 49297) % 233280;
                currX += barW + Math.floor(2 + (seed / 233280) * 4);
            }
        }

        function drawSticker(cCtx, img, x, y, maxW, maxH, rot) {
            rot = rot || 0;
        ...
        async function renderStrip() {
            var placeholder = document.getElementById('strip-placeholder');
            var finalStrip = document.getElementById('final-strip');
            var downloadBtn = document.getElementById('download-btn');

            if (placeholder) {
                placeholder.innerHTML = '<div class="animate-spin rounded-full h-8 w-8 border-b-2 border-espresso mb-3 mx-auto"></div><p class="font-hand text-lg text-cocoa-medium">Sedang merajut strip foto kamu...</p>';
                placeholder.classList.remove('hidden');
            }
            if (finalStrip) finalStrip.classList.add('hidden');
            if (downloadBtn) downloadBtn.classList.add('hidden');

            var headerText = document.getElementById('header_text') ? document.getElementById('header_text').value : 'Capturing Moments';
            var footerText = document.getElementById('footer_text') ? document.getElementById('footer_text').value : 'On the road, 20!';
            var styleTheme = document.getElementById('style_theme') ? document.getElementById('style_theme').value : 'classic_vintage';
            var photoShape = document.getElementById('photo_shape') ? document.getElementById('photo_shape').value : 'square';

            try {
                var stripW = 600;
                var photoW = 440;
                var photoH = (photoShape === 'oval') ? 300 : 330;
                var topPad = 200;
                var gap = 25;
                var botPad = 220;
                var stripH = topPad + (3 * photoH) + (2 * gap) + botPad;
                ...
                if (styleTheme === 'classic_vintage') {
                    ...
                    var cardX1 = 50, cardY1 = 120, cardW = stripW - 100, cardH = stripH - 240;
                    var cardX2 = cardX1 + cardW, cardY2 = cardY1 + cardH;
                    drawRoundedRect(cCtx, cardX1, cardY1, cardW, cardH, 25, '#FDFCFA');

                    var midY = Math.round((cardY1 + cardY2) / 2);
                    cCtx.fillStyle = '#2B1B10';
                    cCtx.beginPath();
                    ...
                    var denimImg = await loadImage('/images/denim_bg_collage.png');
                    if (denimImg) {
                    ...
                    var dCardX1 = 45, dCardY1 = 50, dCardW = stripW - 90, dCardH = stripH - 100;
                    var dCardX2 = dCardX1 + dCardW;
                    
                    drawRoundedRect(cCtx, dCardX1, dCardY1, dCardW, dCardH, 25, '#FCF8EE', '#8B2D2D', 3);
                    drawRoundedRect(cCtx, dCardX1 + 6, dCardY1 + 6, dCardW - 12, dCardH - 12, 20, null, '#8B2D2D', 1);

                    var notchR = 16;
                    var denimBlue = '#325078';
                    [175, stripH - 175].forEach(function(divY) {
                        cCtx.fillStyle = denimBlue;
                        cCtx.beginPath();
                        cCtx.arc(dCardX1, divY, notchR, 0, Math.PI * 2);
                        cCtx.fill();
                        cCtx.beginPath();
                        cCtx.arc(dCardX2, divY, notchR, 0, Math.PI * 2);
                        cCtx.fill();
                        ...
                        cCtx.beginPath();
                        cCtx.moveTo(dCardX1 + 16, divY);
                        cCtx.lineTo(dCardX2 - 16, divY);
                        cCtx.stroke();
                        cCtx.restore();
                    });
                    ...
                    drawBarcode(cCtx, dCardX2 - 100, stripH - 145, 80, 65, '#8B2D2D');
                    ...
                    var flowerImg = await loadImage('/images/flower_sticker_1.png');
                    ...
                    var ppgBg = await loadImage('/images/ppg_bg_collage.jpg');
                    if (ppgBg) {
                    ...
                    var pCardX1 = 50, pCardY1 = 120, pCardW = stripW - 100, pCardH = stripH - 240;
                    var pCardY2 = pCardY1 + pCardH;
                    drawRoundedRect(cCtx, pCardX1, pCardY1, pCardW, pCardH, 25, 'rgba(253, 252, 248, 0.94)', '#5C4033', 2);

                    cCtx.fillStyle = '#5C4033';
                    cCtx.font = 'bold 20px Georgia, serif';
                    cCtx.textAlign = 'center';
                    cCtx.textBaseline = 'middle';
                    cCtx.fillText(footerText, stripW / 2, pCardY2 - 25);
                } else if (styleTheme === 'polaroid_printer') {
                    var polBg = await loadImage('/images/polaroid_bg_collage.jpg');
                    if (polBg) {
                ...
                if (styleTheme !== 'denim_y2k') {
                    var bannerW = 440, bannerH = 70;
                    var bx1 = (stripW - bannerW) / 2, by1 = 35;
                ...
                var currentY = topPad;
                var pasteX = (stripW - photoW) / 2;

                for (var j = 0; j < 3; j++) {
                    var imgData = capturedImages[j];
                    if (!imgData) continue;
                    var img = await loadImage(imgData);
                    if (!img) continue;

                    var targetRatio = photoW / photoH;
                    ...
                    if (styleTheme === 'polaroid_printer') {
                        var frameW = photoW + 30, frameH = photoH + 20;
                        var frameX = (stripW - frameW) / 2;
                    ...
                    var borderColor = (styleTheme === 'denim_y2k') ? '#8B2D2D' : '#5C4033';
                ...
                if (styleTheme === 'ppg_collage') {
                    ...
                    drawSticker(cCtx, ppgImgs[0], 110, 150, 130, 130, -8);
                    drawSticker(cCtx, ppgImgs[1], stripW - 110, 150, 130, 130, 8);
                    drawSticker(cCtx, ppgImgs[3], 85, stripH - 210, 80, 80, -12);
                    drawSticker(cCtx, ppgImgs[2], stripW - 105, stripH - 200, 130, 130, 6);
                }

                var resultUrl = c.toDataURL('image/png');
                if (finalStrip) finalStrip.src = resultUrl;
                if (downloadBtn) downloadBtn.href = resultUrl;
                if (placeholder) placeholder.classList.add('hidden');
                if (finalStrip) finalStrip.classList.remove('hidden');
        ...
        // Global Event Delegation (Aman dari Vite Module timing)
        document.addEventListener('click', async function(e) {
            // 1. Klik Slot Foto
            var slotBtn = e.target.closest('.slot-trigger');
            if (slotBtn) {
                var slotIdx = slotBtn.getAttribute('data-slot');
                var input = document.getElementById('photo-input-' + slotIdx);
                if (input) input.click();
                return;
            }

            // 2. Klik Aktifkan Kamera
            var startCamBtn = e.target.closest('#start-camera-btn');
            if (startCamBtn) {
                try {
                    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                        alert('Akses kamera tidak didukung di browser ini. Silakan gunakan tombol pilih foto di bawah.');
                        return;
                    }
            ...
            // 3. Klik Ambil Foto Otomatis
            var snapBtn = e.target.closest('#snap-btn');
            if (snapBtn) {
                var video = document.getElementById('webcam');
        ...
        // Change Listener (Upload & Input Text/Select)
        document.addEventListener('change', async function(e) {
            // Upload Slot Tunggal
            if (e.target && e.target.classList.contains('slot-file-input')) {
                var file = e.target.files[0];
                if (!file) return;
                var idx = e.target.getAttribute('data-slot');
            ...
            // Upload 3 File Sekaligus
            if (e.target && e.target.id === 'manual-photos-input') {
            ...
            // Live Update Select Option
            if (e.target && e.target.classList.contains('cfg-input')) {
                checkAndRender();
            }
        });

        // Input Listener (Header/Footer Text)
